window.SCORING = (function(){
  ...
  function scoreSectionA(a,b){
    ...
    let matched=0;
    singles.forEach(q=>{ if(a[q] && b[q] && a[q]===b[q]) matched++; });
    const j6 = jaccard(a['q6']||[], b['q6']||[]);
    const j9 = jaccard(a['q9']||[], b['q9']||[]);
  ...
  function scoreSectionB(answers){
    ...
    possible += 5;
    const correctB4 = new Set(['Sanitary pads','Tampons','Menstrual cup']);
    const sel = answers['b4']||[];
    let b4p=0;
    sel.forEach(s=>{ if(correctB4.has(s)) b4p++; });
    points += b4p;
